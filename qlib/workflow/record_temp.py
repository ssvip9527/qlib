...
class RecordTemp:
    ...
    artifact_path = None
    depend_cls = None  # 记录的依赖类；记录将依赖于由`depend_cls`生成的结果
    ...
    def load(self, name: str, parents: bool = True):
        ...
        parents : bool
            每个记录器都有不同的`artifact_path`。
            因此parents会递归地在父类中查找路径
            子类具有更高的优先级
    ...
    def _generate(self, **kwargs):
        ...
        for _analysis_freq in self.risk_analysis_freq:
            if _analysis_freq not in portfolio_metric_dict:
                warnings.warn(
                    f"未找到频率为 {_analysis_freq} 的报告，请将对应环境的 `generate_portfolio_metrics` 设置为 True"
                )
            else:
        ...
        for _analysis_freq in self.indicator_analysis_freq:
            if _analysis_freq not in indicator_dict:
                warnings.warn(f"未找到频率为 {_analysis_freq} 的指标")
            else:
    ...
    def list(self):
        ...
        for _analysis_freq in self.risk_analysis_freq:
            if _analysis_freq in self.all_freq:
                ...
                warnings.warn(f"未找到风险分析频率 {_analysis_freq}")

        for _analysis_freq in self.indicator_analysis_freq:
            if _analysis_freq in self.all_freq:
                list_path.append(f"indicator_analysis_{_analysis_freq}.pkl")
            else:
                warnings.warn(f"未找到指标分析频率 {_analysis_freq}")
        return list_path
