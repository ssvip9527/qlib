...
# ArcticDB使用LMDB本地存储
ARCTICDB_URI = "lmdb:///tmp/arcticdb"
...
def add_one_stock_daily_data(filepath, type, exchange_place, arctic, date):
    """
...
def add_one_stock_daily_data_wrapper(filepath, type, exchange_place, index, date):
    ...
    code = os.path.split(filepath)[-1].split(".csv")[0]
    # 创建ArcticDB连接
    arctic = adb.Arctic(ARCTICDB_URI)
    try:
        if index % 100 == 0:
            print("index = {}, filepath = {}".format(index, filepath))
        error_index_list = add_one_stock_daily_data(filepath, type, exchange_place, arctic, date)
...
def add_data(tick_date, doc_type, stock_name_dict):
    ...
    try:
        ...
        f = (DATA_INFO_PATH / "data_info_log_{}_{}".
            format(doc_type, tick_date)).open("w+")
        ...
        f = (DATA_FINISH_INFO_PATH / "data_info_finish_log_{}_{}".
            format(doc_type, tick_date)).open("w+")
...
class DSCreator:
    ...
    def clear(self):
        # ArcticDB不需要删除数据库，只需删除LMDB文件
        if os.path.exists("/tmp/arcticdb"):
            shutil.rmtree("/tmp/arcticdb")
    ...
    def initialize_library(self):
        ...
        for doc_type in DOC_TYPE:
            # 创建库
            arctic.create_library(get_library_name(doc_type))
    ...
    def import_data(self, doc_type_l=["Tick", "Transaction", "Order"]):
        ...
        for doc_type in doc_type_l:
            ...
            # 获取已存在的股票代码
            stock_name_exist = lib.list_symbols()
            initialize_count = 0
